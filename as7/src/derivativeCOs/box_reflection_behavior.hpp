...
namespace CO{
    class BoxReflectionBehavior : public Behavior {
        ...
        public:
            ...
            BoxReflectionBehavior(Entity* target, std::shared_ptr<std::vector<std::shared_ptr<Entity>>> collisionEntities) : Behavior(target), collisionEntities(collisionEntities) { // Constructor taking in a target entity and a list of collision entities
            ...
            void Tick(const float& deltaTime) override { // Tick function to be called every frame
                if(enabled){ // Check if the behavior is enabled
                    if(target->hasComponent<CO::BoxCollisionComponent>() && target->hasComponent<CO::KinematicsComponent>()) { // Check if the target entity has a box collision component and kinematics component
                        ...
                        collision_vector = CheckCollisionBoundary(targetRect, worldBoundary, targetVelocity); // Check for collision with the world boundary
                        ...
                        targetPosition.x = std::fmax(0, std::fmin(worldBoundary.width - targetRect.width, targetPosition.x)); // Clamp the target position to the world boundary
                        targetPosition.y = std::fmax(0, std::fmin(worldBoundary.height - targetRect.height, targetPosition.y)); // Clamp the target position to the world boundary
                        ...
                        for(auto& entity : *collisionEntities) { // Loop through the collision entities
                            if(entity->hasComponent<CO::BoxCollisionComponent>() && entity->hasComponent<CO::KinematicsComponent>()) { // Check if the entity has a box collision component and kinematics component
                                ...
                                Vector2 current_collision = CheckCollisionBoundary(targetRect, entityRect, targetVelocity); // Check for collision with the entity
